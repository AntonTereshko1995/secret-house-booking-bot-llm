...
async def test_pricing_functionality():
    ...
    try:
        ...
        request = PricingRequest(
            tariff_id=0, duration_days=1, add_ons=["sauna", "secret_room"]
        )
        response = await pricing_service.calculate_pricing(request)
        ...
        for query in test_queries:
            ...
            print(
                f"   ✓ '{query}' -> tariff: '{request.tariff}', days: {request.duration_days}"
            )
...
async def test_russian_pricing_queries():
    ...
    for query in test_queries:
        ...
        request = await extractor.extract_pricing_requirements(query)
        print(
            f"  Extracted: tariff='{request.tariff}', days={request.duration_days}, add_ons={request.add_ons}"
        )
